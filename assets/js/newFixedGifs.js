...
const defaultOrder = ['include-title', 'include-description', 'include-source', 'include-author', 'include-category', 'include-pubDate'];
...
function updateOrderArray() {
    ...
    $("input[type=checkbox].updateElement:checked").each(function () {
...
function updateSelectedElements() {
    $("#selected_elements").empty().toggle($("input[type=checkbox].updateElement:checked").length > 0);
...
$(document).ready(function () {
    ...
    $("input[type=checkbox].updateElement").change(function () {
...
function displayRSSInfo(xmlDoc) {
    ...
    if (items.length > 0) {
        ...
        infoContainer.innerHTML = "<p>Error retrieving RSS feed information: Here are the different elements that make up the RSS feed that you can use for the GIF:</p>";
...
document.getElementById('update-weekly').addEventListener('change', function() {
    document.getElementById('update-daily').checked = !this.checked;
});

document.getElementById('update-daily').addEventListener('change', function() {
    document.getElementById('update-weekly').checked = !this.checked;
});